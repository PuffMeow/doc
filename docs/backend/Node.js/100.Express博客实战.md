...
- 安装依赖：`npm install express sequelize mysql2 body-parser cors bcrypt jsonwebtoken`
  - express: 我们要用的服务端框架
...
这里可以启动自己的本地 mysql 数据库，然后填入对应信息即可，不知道怎么安装 mysql 的可以网上查下相关教程，可以使用 docker 安装，或者直接用安装包装到本地。
