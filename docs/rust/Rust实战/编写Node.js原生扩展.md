...
啥，你说为啥不选用别的语言支撑前端基建而用 Rust？
从别的语言方面来看，C/C++ 不是内存安全的，对于内存的掌控更加难，很容易就会栽在内存方面找大半天的bug; Java 有虚拟机和垃圾收集器，并不适合系统级编程; Go 比较适合，毕竟一开始就说要对标 C 语言，但是它也是带着垃圾回收的，Deno 在一开始就是用 Go 来写的，后续因为 Go 的垃圾回收会造成全停顿，还有一些其它的原因，最后改成了使用 Rust 实现。
从 Rust 本身来看，Rust 并没有外界宣称的那么的难，上手之后开发效率并不低，一些最难最复杂的部分在日常应用层面开发基本都不会接触到，比如写一个链表什么的都有现成封装好的库了，另外很多应用层框架都已经帮你处理掉生命周期的事情了，并且 Rust 没有垃圾回收，执行效率和 C++ 一个等级。在跨语言调用来看，它也比别的语言要更好，同时它拥有类似于前端的工具链 cargo 以及和 TypeScript 相似的类型系统，如下:
...
- [@puffmeow/rusty-schema2ts](https://github.com/PuffMeow/rusty-schema2ts)：使用纯 Rust 写的，将 json-schema 转换成 TS 类型的库。
